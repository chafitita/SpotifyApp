...
import { CLIENT_ID, REDIRECT_URI, AUTH_ENDPOINT, RESPONSE_TYPE, SCOPES } from '../components/config';
...
function home(){
    ...
    const handleLogin = () => {
        const authUrl = `${AUTH_ENDPOINT}?client_id=${CLIENT_ID}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&response_type=code&scope=${encodeURIComponent(SCOPES)}`;
    ...
    <div className="app-container">
        ...
        <main className="main-content" style={{ textAlign: "center", marginTop: "20px" }}>
            {!isLoading && (
            <button onClick={handleLogin} className="login-button">Login con Spotify</button>
            )}
        </main>
        <footer>
